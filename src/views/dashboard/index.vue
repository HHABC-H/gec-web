  ...
  <div class="ai-qa-container">
    ...
    <div class="header">
      ...
      <div class="header-actions">
        <button class="clear-btn" title="清空对话" @click="clearChat">
          <i class="icon-clear" /> 清空对话
        </button>
      </div>
    </div>
        ...
        <div class="messages-list">
          ...
          >
            ...
            <div v-if="msg.type === 'ai'" class="message-content ai">
              ...
              <div v-if="!msg.isStreaming && msg.text" class="message-footer">
                <button
...
<script>
import { fetchChatStream } from '@/api/ocean-ai'
...
export default {
  ...
  methods: {
    ...
    async streamAIResponse(question, messageIndex) {
      try {
        const response = await fetchChatStream(question, this.abortController.signal)
...
.welcome-card {
  ...
  margin: 0 auto;
  padding: 40px;
  background: linear-gradient(135deg, #f8fdff 0%, #e8f4fe 100%);
  border-radius: 20px;
...
.ai-message:hover .message-footer {
  opacity: 1;
}
